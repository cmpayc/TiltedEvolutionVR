...
#if TP_SKYRIMVR
    // VR reads its addresses from the table generated by Tools/vr_addresses/resolve.mjs, which
    // is compiled into the binary. Nothing needs to be installed next to the game, and an id
    // with no VR address is caught where it is used instead of silently becoming null.
    bool LoadGeneratedVRTable(int major, int minor, int revision, int build)
    {
        Clear();
        ...
        const VRAddress* pEntries = VRAddresses::Data();
        for (size_t i = 0, count = VRAddresses::Count(); i < count; ++i)
        {
            if (pEntries[i].vrOffset == 0)
                continue;

            _data[pEntries[i].aeId] = pEntries[i].vrOffset;
...
#if TP_SKYRIMVR
        return LoadGeneratedVRTable(major, minor, revision, build);
#else
...
#if TP_SKYRIMVR
            // Fail here, while the id is still known, rather than let a null propagate into a
            // patch or a call and crash somewhere unrelated.
            if (m_pPtr == nullptr)
                VRAddresses::ReportMissing(m_id);
#endif
        }
