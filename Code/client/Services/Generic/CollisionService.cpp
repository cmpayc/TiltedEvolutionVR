 ...
 * holding. Only these are kept off clutter and off weapons.
 *
 ...
 * contact that only happens when somebody is swinging.
 */
...
// The row a layer is mirrored onto for the local player, or 0 if it has none.
uint32_t MirrorOf(const uint32_t aLayer) noexcept
{
    for (size_t i = 0; i < kActorLayerCount; ++i)
        if (aLayer == kActorLayers[i])
            return s_mirrorLayers[i];
...
void PatchLayerTable(uint64_t* apTable) noexcept
{
     ...
     *
     * The whole body list is kept off clutter and off weapons, and the weapon layer is kept off clutter on its
     * own, since the body list deliberately leaves it out.
     *
     * **The weapon against the character controller is how the player lands a hit**, measured the hard way on
     * 2026-09-12: clearing it while the player's melee bodies were still on layer 5 left every player unable to
     * touch an NPC. It can be closed because those bodies now go onto the weapon's mirror, which keeps the pair,
     * see FindMeleeBodies. Reported 2026-09-13 with it open: another player's capsule brushing a sword on the
     * floor sent it drifting, while clutter stayed put. Without a mirror row for the weapon it stays open.
     */
    ...
    for (const uint32_t cBodyLayer : kActorBodyLayers)
    {
        ...
        if (cBodyLayer != kCharControllerLayer || MirrorOf(kWeaponLayer))
    ...
    if (s_mirrorLayers[0])
        spdlog::info(
            "[clutter] characters no longer shove clutter or weapons about. The local player's controller mirror, layer {}, has mask {:#018X}, and still meets clutter {}, weapons {}, controllers {} and limbs with no controller {}. Controllers meet weapons {}, and the player's weapon mirror, layer {}, meets controllers {}",
            ...
            (apTable[s_mirrorLayers[0]] >> 33) & 1, (apTable[kCharControllerLayer] >> kWeaponLayer) & 1, MirrorOf(kWeaponLayer), MirrorOf(kWeaponLayer) ? (apTable[MirrorOf(kWeaponLayer)] >> kCharControllerLayer) & 1 : 0);
    else
...
/**
 * @brief The havok bodies of the local player's VR melee weapons, at most one per hand. Returns how many.
 *
 * These are what the player lands a hit with, the weapon body against the character controller, and they share
 * layer 5 with every sword lying on the floor. Neither the controller search nor the 3D walk finds them, because
 * the game hangs them under PlayerWorldNode, and that is why closing the controller against weapons on
 * 2026-09-12 left every player missing every NPC: these bodies were still on 5.
 *
 * Read out of SkyrimVR 1.4.15 on 2026-09-13, from the function that builds them at 0x1406B03D0 and the one that
 * tears them down at 0x1406B0D50. The player keeps a 0xD0 byte melee record per hand from +0x710, each holding
 * its collision node at +0x18. The node keeps its collision object at +0x40, that keeps the bhkRigidBody at
 * +0x20, and the builder gives the body filter info (group << 16) | 0x4085, which is layer 5.
 *
 * Plain loads, so this can run every frame. Each link is a reference the game holds and nulls when it lets go,
 * written on the main thread this runs on. The type is checked by RTTI only when a hand's body pointer changes.
 */
size_t FindMeleeBodies(PlayerCharacter* apPlayer, const uint8_t* aBodies[2]) noexcept
{
    constexpr size_t kMeleeDataOffset = 0x710;
    constexpr size_t kMeleeDataSize = 0xD0;
    constexpr size_t kCollisionNodeOffset = 0x18;
    constexpr size_t kCollisionObjectOffset = 0x40;
    constexpr size_t kBodyOffset = 0x20;
    constexpr size_t kReferencedObjectOffset = 0x10;

    static const uint8_t* s_checked[2]{};
    static bool s_isRigidBody[2]{};
    ...
    {
        const uint8_t* cpNode = cLoad(reinterpret_cast<const uint8_t*>(apPlayer) + kMeleeDataOffset + hand * kMeleeDataSize + kCollisionNodeOffset);
        const uint8_t* cpCollision = cpNode ? cLoad(cpNode + kCollisionObjectOffset) : nullptr;
        const uint8_t* cpBhkBody = cpCollision ? cLoad(cpCollision + kBodyOffset) : nullptr;

        if (!cpBhkBody)
            continue;

        if (cpBhkBody != s_checked[hand])
        {
            s_checked[hand] = cpBhkBody;
            s_isRigidBody[hand] = IsInstanceOf(cpBhkBody, "bhkRigidBody");

            if (!s_isRigidBody[hand])
                spdlog::warn("[clutter] melee record {} of the local player does not lead to a bhkRigidBody, so that weapon stays on layer {} and cannot hit anyone", hand, kWeaponLayer);
        }

        if (const uint8_t* cpWorldObject = s_isRigidBody[hand] ? cLoad(cpBhkBody + kReferencedObjectOffset) : nullptr)
            aBodies[count++] = cpWorldObject;
    }
...
/**
 * @brief Moves the local player's melee weapon bodies onto the weapon's mirror on the frame the game hands them
 *        back on layer 5.
 *
 * Every frame, because the game rebuilds these bodies from sixteen call sites, drawing, sheathing and equipping
 * among them, and often at the address it just freed, so no pointer compare can tell a new body from the old one.
 * Only the body's layer can, and a body left on 5 until the backstop comes round is a player who misses
 * everything they swing at. The guarded move runs only on a frame that plain load reads 5.
 */
void KeepMeleeBodiesMirrored(PlayerCharacter* apPlayer) noexcept
{
    constexpr size_t kCollisionFilterOffset = 0x4C;
    constexpr uint32_t kLayerMask = 0x7F;

    if (!s_playerMirrored || !MirrorOf(kWeaponLayer))
        return;

    const uint8_t* bodies[2]{};
    const size_t cCount = FindMeleeBodies(apPlayer, bodies);

    for (size_t i = 0; i < cCount; ++i)
    {
        if ((*reinterpret_cast<const uint32_t*>(bodies[i] + kCollisionFilterOffset) & kLayerMask) != kWeaponLayer || !MoveBodyToMirror(bodies[i], true))
            continue;

        // Counted, so a count that climbs with the frame rate says the game rewrites the layer every frame.
        static uint32_t s_moves = 0;

        if (++s_moves <= 4 || s_moves % 1000 == 0)
            spdlog::info("[clutter] a melee weapon body of the local player was back on layer {} and is on row {} again, {} times so far", kWeaponLayer, MirrorOf(kWeaponLayer), s_moves);
    }
}
...
bool MirrorLocalPlayerCollision() noexcept
{
    ...
    const uint8_t* meleeBodies[2]{};
    const size_t cMeleeCount = FindMeleeBodies(pPlayer, meleeBodies);

    bodies.insert(bodies.end(), meleeBodies, meleeBodies + cMeleeCount);
...
void CollisionService::OnUpdate(const UpdateEvent& acEvent) noexcept
{
    ...
    {
        KeepMeleeBodiesMirrored(pPlayer);
        return;
    }
